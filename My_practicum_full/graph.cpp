...
void graph::building()
{
       ...
       QDataStream stream(file);  // create stream for file
       int i=0;
       QLineSeries * series = new QLineSeries;

    file->open(QFile::ReadWrite); // prepare file for read and write
    while(!file->atEnd()) // if file isn't ending work with all stream consistently,
    {
        stream >> vecsort; // add file data to vector
        if (vecsort[0] == edit->text()) // if entered name exist in file
        {
            masvec[i] = vecsort;
            i++;
                // eventually "masvec" contain /entered name/ file data
        }
    }
